...
impl<T: UserEvent> Context<T> {
  fn create_window<F: Fn(RawWindow) + Send + 'static>(
    ...
    // Do not let Tauri attach a descriptor for a window that failed to create.
    completion_rx
      .recv()
  ...
  match message {
    ...
    Message::CreateWebview(window_id, handler, completion) => {
      ...
      if let Some((Some(window), focused_webview)) = window {
        ...
        let _ = completion.send(Err(Error::WindowNotFound));
      }
    }
    Message::CreateWindow(window_id, handler, completion) => {
      complete_creation(handler(event_loop), completion, |webview| {
...
fn complete_creation<C>(
  result: Result<C>,
  completion: Sender<Result<()>>,
  install: impl FnOnce(C) -> Result<()>,
) {
  let _ = completion.send(result.and_then(install));
}

#[cfg(test)]
mod creation_completion_tests {
  use super::*;

  #[test]
  fn failed_creation_returns_error_without_installing_a_phantom_window() {
    let (tx, rx) = channel();
    let mut installed = false;
    complete_creation::<()>(Err(Error::CreateWindow), tx, |_| {
      installed = true;
      Ok(())
    });
    assert!(matches!(rx.recv().unwrap(), Err(Error::CreateWindow)));
    assert!(!installed);
  }

  #[test]
  fn creation_is_installed_before_the_waiting_caller_is_released() {
    let (tx, rx) = channel();
    let mut window = None;
    complete_creation(Ok(42), tx, |created| {
      assert!(matches!(
        rx.try_recv(),
  ...
  #[test]
  fn missing_parent_during_installation_is_reported_as_failure() {
    let (tx, rx) = channel();
    complete_creation(Ok(()), tx, |_| Err(Error::WindowNotFound));
    assert!(matches!(rx.recv().unwrap(), Err(Error::WindowNotFound)));
  }

  #[test]
  fn webview_error_details_reach_the_caller() {
    let (tx, rx) = channel();
    let result = Err(Error::CreateWebview(Box::new(std::io::Error::other(
      "controller failed",
    ))));
    complete_creation::<()>(result, tx, |_| panic!("failed webview was installed"));
    assert!(rx
      .recv()
      .unwrap()
      .unwrap_err()
      .to_string()
      .contains("controller failed"));
  }
}
...
"#,
  ...
  .map_err(|e| {
    // No WebviewWrapper will be constructed to release this reference on error.
    web_context.referenced_by_webviews.remove(&label);
    Error::CreateWebview(Box::new(e))
  })?;
